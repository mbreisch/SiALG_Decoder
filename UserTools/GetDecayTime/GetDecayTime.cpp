...
bool GetDecayTime::Execute()
{
    ...
    if(m_data->TD.EndOfRun==true)
    {
        for(const auto& pair : m_data->TD.TTS_Map) 
        {
            ...
            auto it = m_data->TD.Decay_Map.find(key);
            if(it != m_data->TD.Decay_Map.end()) 
            {
            ...
            }else 
            {
                ...
                m_data->TD.Decay_Map[key] = vector2;
            }
        }
        std::cout <<"Run-End has been called, Decay calculator now has "<<m_data->TD.Decay_Map.size()<<" entries"<<std::endl;
        for(const auto& pair : m_data->TD.TTS_Map) 
        {
...
void GetDecayTime::FitDecay(int channel, vector<float> data)
{
    ...
    {
        fit_function = new TF1("fit_function", Sum_of_Exp_Normal, fit_start, fit_end, 10); // 8 is the number of parameters
    }else
    {
        return;
    }
    ...
    // Print the results
    printf("%s\n","---------------");
    printf("Fit result, channel %i: sigma = %f, t0 = %f, n0 = %f, tau0 = %f, n1 = %f, tau1 = %f, n2 = %f, tau2 = %f, n3 = %f, tau3 = %f\n", channel, 
           fit_sigma, fit_t0, fit_n_0, fit_tau_0, fit_n_1, fit_tau_1, fit_n_2, fit_tau_2, fit_n_3, fit_tau_3);
    printf("%s\n","---------------");
