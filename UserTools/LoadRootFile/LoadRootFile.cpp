...
bool LoadRootFile::Execute()
{
    ...
    if(m_verbose>2){std::cout << "Get current folder " << current_file << std::endl;}
    std::string current_file_saved = current_file;
    current_file += "Analysis.root";
    if(m_verbose>2){std::cout << "Get current file " << current_file << std::endl;}
    
    TFile* TTfile = TFile::Open(current_file.c_str(), "READ");
    if(!TTfile || TTfile->IsZombie()) 
    {
        if(m_verbose>1){std::cerr << "Error opening file: " << current_file << std::endl;}
        m_data->TD.Stop=true;
        return true;
    }
    if(!m_variables.Get("Path_Out",m_data->TD.Path_Out)) m_data->TD.Path_Out=current_file_saved;

    TTree* current_tree = dynamic_cast<TTree*>(TTfile->Get(Treename.c_str()));
    //TTfile->GetObject(Treename.c_str(),current_tree);
    if(!current_tree) 
    {
        if(m_verbose>1){std::cerr << "Error accessing tree in file." << std::endl;}
        TTfile->Close();
        return true;
    }      
    ...
    for(int i_ch=0; i_ch<MAX_NUM_CHANNEL; i_ch++)
    {
        ...
        if(m_verbose>2){std::cout<<"I want to read "<<Branchname_current<<std::endl;}
        ...
        {
            if(m_verbose>2){std::cout<<"I want to read "<<i<<" from a total of "<<current_tree->GetEntries()<<std::endl;}
            current_tree->GetEntry(i);
            if(data) 
            {
                dataVector = (*data);
            }else {
                if(m_verbose>1){std::cerr << "Error reading data from branch " << Branchname_current << std::endl;}
                TTfile->Close();
                return false;
            }
        }
        if(m_verbose>2){std::cout<<"For "<<i_ch<<" "<<dataVector.size()<<" was written"<<std::endl;}
