...
"""
def GetCalibrationCurve(AllDictonary,print_single_volt,id,channelmap):
    pedestal = []
    spe = []
    dpe = []
    err_pedestal = []
    ...
    for run,data in AllDictonary.items():
        if channelmap == -1:
            ...
            meas_channel = int(run-1)

        rows = run_index // 4
        cols = run_index % 4
        ldata = data["TTree_Analysis_PHD;1"]["PulseHeight_Ch{}".format(meas_channel)]
        bin_width = 1/4.096
        if print_single_volt==True:
...
def GetChargeSpectra(AllDictonary):
    pedestal = []
    spe = []
    dpe = []
    err_pedestal = []
    ...
    run_index = 0
    for enum,(channel,data) in enumerate(AllDictonary["TTree_Analysis_PHD;1"].items()):
        rows = run_index // 4
        cols = run_index % 4
        ldata = np.array(data)
        bin_width = 1/4.096
        axes[rows, cols].set_title("Channel {}".format(channel))
        axes[rows, cols].hist(ldata,bins=np.arange(min(ldata),max(ldata)+bin_width,bin_width),label="Data",alpha=0.5,log=True)
        axes[rows, cols].set_xlim([-100,0])
        axes[rows, cols].set_ylim([0.1,20000])
        axes[rows, cols].set_xlabel("Pulse Height [ADC]")
        axes[rows, cols].set_ylabel("Counts")
        ...
        # Step 3: Identify local maxima
        p,_ = find_peaks(smoothed_histogram,distance=25,height=50)
    ...
    return pedestal,spe,dpe,err_pedestal,err_spe,err_dpe